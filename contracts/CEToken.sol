...
contract CEToken is ERC20, Ownable {
    ...
    /**
     * @dev Allows the contract owner to withdraw any CE tokens that were
     * mistakenly sent to this contract's address.
     */
    function withdrawStuckCE() public onlyOwner {
        uint256 balance = balanceOf(address(this));
        require(balance > 0, "CEToken: No CE tokens to withdraw from contract");
    ...
    function withdrawStuckERC20(IERC20 tokenContract) public onlyOwner {
        require(address(tokenContract) != address(0), "CEToken: Token address cannot be zero");
        uint256 balance = tokenContract.balanceOf(address(this));
        require(balance > 0, "CEToken: No tokens of this type to withdraw from contract");
        bool success = tokenContract.transfer(owner(), balance);
        require(success, "CEToken: ERC20 token transfer failed");
    }
}